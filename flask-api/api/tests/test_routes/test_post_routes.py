...
class TestDB:
    ...
    def test_create_post(self, client, db):
        response = client.post(
            "/api/post/create/",
            data={"title": "My Post Title", "content": "My Post Content"},
        )
    ...
    def test_get_single_post(self, client, db):
        response = client.get("/api/post/<int:id>", data={"id": 1})
    ...
    def test_update_post(self, client, db):
        response = client.put(
            "/api/post/update/",
            data={"id": 1, "title": "New Title", "content": "This is the new content."},
        )
        assert response.status_code == 200
        """
